...
def generate_random_transform(params, shape):
    ...
    if params.image_dimension == 2:
      ...
      theta = np.pi / 180.0 * np.random.uniform(-params.rotation_range, params.rotation_range)

      rotation[:2,:2] = dv.rotation_matrix_from_angle(theta)
    elif params.image_dimension == 3:
      x = dv.rotation_matrix_from_angle(np.pi/180.0 * np.random.uniform(-params.rotation_range[0], params.rotation_range[0]), matrix_type = 'roll')
      y = dv.rotation_matrix_from_angle(np.pi/180.0 * np.random.uniform(-params.rotation_range[1], params.rotation_range[1]), matrix_type = 'pitch')
      z = dv.rotation_matrix_from_angle(np.pi/180.0 * np.random.uniform(-params.rotation_range[2], params.rotation_range[2]), matrix_type = 'yaw')
