...
def wrapped_phase_difference(y_true, y_pred):
  ...
  diff = y_pred - y_true
  diff -= tf.multiply(tf.cast(tf.greater_equal(diff, dvpy.tf.pi), 'float32'), 2.0*dvpy.tf.pi)
  diff += tf.multiply(tf.cast(tf.less(diff, -dvpy.tf.pi), 'float32'), 2.0*dvpy.tf.pi)
  return diff
