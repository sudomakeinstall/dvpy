...
def transform_full_matrix_offset_center(matrix, shape):
    ...
    if matrix.shape[0] != (len(shape) + 1):
      raise ValueError('The first dimension of the transformation matrix must be equal to len(shape) + 1.')

    if matrix.shape[1] != (len(shape) + 1):
      raise ValueError('The second dimension of the transformation matrix must be equal to len(shape) + 1.')
...
def transform_partial_matrix_offset_center(matrix, shape):
    ...
    if matrix.shape[0] != len(shape):
      raise ValueError('The first dimension of the transformation matrix must be equal to len(shape).')
