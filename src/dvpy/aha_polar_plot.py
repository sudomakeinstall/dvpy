...
def aha_polar_plot(ax, B, E, A, P,
  ...
  thetas = [(i+1)*TWO_PI/6 for i in range(6)]*2 + [(i+0.5)*TWO_PI/4 for i in range(4)] + [0]
  heights = [2./7]*16 + [1./7]
  widths = [TWO_PI/6]*12 + [TWO_PI/4]*4+[TWO_PI]
  inner_radii = [bounding_radius*5./7]*6 + [bounding_radius*3./7]*6 + [bounding_radius*1./7]*4 + [0]
  ...
  if B is not None:
    ax.bar(np.array(thetas[0:6]) + np.pi / 6.0,
             heights[0:6],
             width=widths[0:6],
  ...
  if E is not None:
    ax.bar(np.array(thetas[6:12]) + np.pi / 6.0,
             heights[6:12],
             width=widths[6:12],
  ...
  if A is not None:
    ax.bar(np.array(thetas[12:16]) + np.pi / 4.0,
             heights[12:16],
             width=widths[12:16],
  ...
  ax.text(np.pi / 2, ost, 'Ant',
            bbox=bbox_props,
            ha='center',
            va='center',
           )
  ax.text(np.pi, ost, 'Sep',
            bbox=bbox_props,
            ha='center',
            va='center',
           )
  ax.text(3 * np.pi / 2, ost, 'Inf',
            bbox=bbox_props,
            ha='center',
            va='center',
           )
