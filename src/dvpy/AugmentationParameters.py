...
def expand_if_scalar(maybe_scalar, dimension):
  if np.isscalar(maybe_scalar):
      ...
      raise Exception('maybe_scalar should be either a float, or an array-like length dimension')
...
class AugmentationParameters():
  ...
  def __init__(
    ...
    self.translation_range = expand_if_scalar(translation_range, self.image_dimension)
    if np.isscalar(scale_range):
      self.scale_range = scale_range
    else:
      raise Exception('scale_range must be a float (only rigid scaling is supported)')
    ...
    if self.image_dimension == 2:
      if not np.isscalar(rotation_range):
        raise Exception('rotation_range should be a float when image_dimension is 2')
      ...
      if np.isscalar(rotation_range):
          ...
          raise Exception('rotation_range should be either a float, or an array-like length image_dimension when image_dimension is 3')
    else:
      raise Exception('Only image dimensions 2 and 3 are supported')

  def __str__(self):

    return '\n'.join(['{} : {}'.format(attr, getattr(self, attr)) for attr in self.__slots__])
